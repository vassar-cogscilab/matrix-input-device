...
##extract raw data##
data.all<-read.csv('C:/Users/research/Downloads/all-data.csv', header=T)
...
## using differnece between N adn W response times as measure for fitting##

data.for.model$difference<-as.vector(mapply(function(x,y){if(!is.na(x)&& !is.na(y)){return(x-y)} else{return(NA)}},data.for.model$rt.N, data.for.model$rt.W))


##organize data by subject, discard NAs##
data<-subset(data.for.model,!is.na(data.for.model$difference))
